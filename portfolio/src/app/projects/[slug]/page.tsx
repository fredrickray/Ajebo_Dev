...
const projectsData: Record<string, {
  ...
  'openrdb-studio': {
    ...
    subtitle: 'Free, open-source, cross-platform desktop GUI for relational databases — Rust, Tauri, React, TypeScript, SQLx.',
    description:
      'Free, open-source, cross-platform desktop GUI for relational databases using Rust, Tauri, React, and TypeScript. Native SQL client with SQLx for type-safe, async database access — an unrestricted alternative to closed-source tools.',
    ...
    architecture: {
      description:
        'Tauri shell hosts a React/TypeScript UI; Rust backend uses SQLx for type-safe async connectivity and query execution across relational databases.',
      nodes: [
        { name: 'React Frontend', description: 'React + TypeScript' },
        { name: 'Rust Backend', description: 'Rust + SQLx' },
        { name: 'Tauri', description: 'Cross-platform desktop shell' },
        { name: 'SQLx', description: 'Type-safe async SQL access' },
        { name: 'Tokio', description: 'Async runtime for Rust' },
        { name: 'Monaco Editor', description: 'SQL editor surface' },
      ],
    },
    challenges: [
      { title: 'Cross-platform desktop shell', description: 'Shipped a native-feeling SQL client via Tauri while keeping a modern React UI.' },
      { title: 'Type-safe DB access', description: 'Used SQLx for async, type-safe connectivity and query execution in Rust.' },
      { title: 'Unrestricted DX', description: 'Designed the product as an open alternative to closed-source database GUIs.' },
    ],
    tradeoffs: [
      { tech: 'Tauri', alternative: 'Electron', reason: 'Smaller footprint and Rust-native backend for a desktop SQL client.' },
      { tech: 'SQLx', alternative: 'ORM-only', reason: 'Type-safe async SQL without locking into a heavy ORM for multi-DB support.' },
      { tech: 'Rust + React', alternative: 'Pure web app', reason: 'Desktop performance and local DB connectivity with a familiar UI stack.' },
    ],
  },
  'marketlense': {
    id: 2,
    slug: 'marketlense',
    title: 'MarketLens AI',
    subtitle: 'AI-powered stock decision support from market trends and real-world signals.',
    ...
    sections: ['System Architecture', 'Engineering Challenges', 'Database Design', 'Trade-offs & Decisions'],
    architecture: {
      description: 'Signal ingest feeds Random Forest and LightGBM models; FastAPI serves predictions; PostgreSQL stores market data.',
      nodes: [
        { name: 'React Frontend', description: 'React + TypeScript' },
...
CREATE INDEX idx_queries_hash ON queries(sql_hash);`,
  ...
  'marketlens': {
    ...
    title: 'MarketLens AI',
    subtitle: 'AI-powered stock decision support from market trends and real-world signals.',
    description: 'Built an AI-powered stock decision support system that analyzes market trends and real-world signals using FastAPI and Python. Trained and deployed Random Forest and LightGBM models, with PostgreSQL for financial/market data.',
    tags: ['FastAPI', 'Python', 'PostgreSQL', 'Random Forest', 'LightGBM'],
    githubUrl: 'https://github.com/fredrickray/marketlense',
    sections: ['System Architecture', 'Engineering Challenges', 'Database Design', 'Trade-offs & Decisions'],
    architecture: {
      description: 'Signal ingest feeds Random Forest and LightGBM models; FastAPI serves predictions; PostgreSQL stores market data.',
      nodes: [
        { name: 'Signal Ingest', description: 'Market & real-world signals' },
        { name: 'FastAPI', description: 'Decision / inference API' },
        { name: 'ML Models', description: 'Random Forest + LightGBM' },
        { name: 'PostgreSQL', description: 'Financial & market data' },
      ],
    },
    challenges: [
      { title: 'Real-world signal fusion', description: 'Combined market trends with external signals for timely, comparable model inputs.' },
      { title: 'Model training & deploy', description: 'Trained Random Forest and LightGBM and served them through FastAPI.' },
      { title: 'Data layer design', description: 'PostgreSQL storage for financial/market data used as model input.' },
    ],
    tradeoffs: [
      { tech: 'PostgreSQL', alternative: 'MongoDB', reason: 'Relational integrity and queries over symbols and signals.' },
      { tech: 'FastAPI', alternative: 'Flask', reason: 'Async support and clear API contracts for ML endpoints.' },
      { tech: 'LightGBM', alternative: 'XGBoost', reason: 'Fast training with strong tabular performance.' },
    ],
  },
  'echoloc': {
    id: 6,
    slug: 'echoloc',
    title: 'EchoLoc',
    subtitle: 'Privacy-first live location sharing with timed sessions.',
    description: 'Privacy-first live location sharing (Expo/React Native). Share location with a group only for a user-defined time window. Group invites, in-group chat, live map, and email/Google/Apple auth over Socket.IO with Echo_API.',
    ...
    architecture: {
      description: 'Mobile clients use Echo_API for auth/groups/sessions and Socket.IO for live locations and chat. Timed sessions auto-end on expiry or manual stop.',
      nodes: [
        { name: 'Expo App', description: 'React Native client' },
        { name: 'Echo_API', description: 'Custom Node backend' },
        { name: 'Socket.IO', description: 'Live location & chat' },
        { name: 'Auth', description: 'Email · Google · Apple' },
        { name: 'Timed Sessions', description: 'Start / stop / expiry' },
      ],
    },
    challenges: [
      { title: 'Timed privacy model', description: 'Sharing scoped to a user-defined window — not always-on tracking.' },
      { title: 'Live group map', description: 'Real-time member locations and chat over Socket.IO while sessions are active.' },
      { title: 'Multi-provider auth', description: 'Email plus Google/Apple sign-in for mobile onboarding.' },
    ],
    tradeoffs: [
      { tech: 'Socket.IO', alternative: 'Polling', reason: 'Low-latency location and chat for active sessions.' },
      { tech: 'Expo', alternative: 'Bare RN', reason: 'Faster mobile delivery with a custom API backend.' },
      { tech: 'Timed sessions', alternative: 'Always-on share', reason: 'Privacy-first product constraint.' },
    ],
  },
  'neuroqo': {
    id: 3,
...
CREATE INDEX idx_records_category ON civic_records(category);`,
  ...
  'propspacex-gateway': {
    ...
    description: 'API gateway for PropSpaceX — a Web2/Web3 real-estate microservice platform with hybrid auth, rate limiting, and routing across gRPC services.',
